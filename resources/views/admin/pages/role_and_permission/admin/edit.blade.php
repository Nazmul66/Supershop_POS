                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" name="branch_name[]" id="branch_name" multiple="multiple">
                                @foreach ($branch as $row)
                                    <option value="{{ $row->id }}" {{ in_array($row->id, $admin->current_branch_id ?? []) ? 'selected' : '' }}>{{ $row->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" name="device_name[]" id="device_name" multiple="multiple">
                                @foreach ($device as $row)
                                    <option value="{{ $row->id }}" {{ in_array($row->id, $admin->current_device_id ?? []) ? 'selected' : '' }}>{{ $row->device_name }}</option>
                                @endforeach
                            </select>
...
@push('add-js')
    ...
    <script>
        ...
        function syncSelectAll(checkboxId, selectId) {
            let total = $(selectId + ' option').length;
            let selected = $(selectId).val() ? $(selectId).val().length : 0;
            $(checkboxId).prop('checked', total === selected);
        }

        syncSelectAll('#selectAllDevices', '#device_name');
        syncSelectAll('#selectAllBranches', '#branch_name');
        syncSelectAll('#selectAllRoles', '#role');
    </script>
